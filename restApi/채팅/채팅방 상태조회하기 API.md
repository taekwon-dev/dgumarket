...
`isWarn`(boolean) : 상대방이 경고 받고 있는 지 유무(defalut = false)
...
** 만약 `isWarn` : true, `block_status` : 1
일 경우라면 어떻게 UI를 표현해야 할까
`isWarn` 에 따른 경고표시는 별개로 나두고 `block_status` 에 따른 차단관련UI는 역시 따로 둔다.
...

{
    ...
    "message": "관리자로 부터 이용제재 받고 있는 유저입니다.",
    "requestPath": "uri=/api/chatroom/444/status",
    "pathToMove": null
}
