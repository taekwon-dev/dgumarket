...
public class ShopValidateImpl {
    ...
    public void shopValidateCheck(JoinPoint joinPoint) throws CustomControllerExecption{
        ...
        if (targetMember == null || targetMember.getIsWithdrawn() == 1) throw new CustomControllerExecption("존재하지 않거나 탈퇴한 유저 입니다.", HttpStatus.BAD_REQUEST, "/", 102);
        if (targetMember.getIsEnabled() == 1) throw new CustomControllerExecption("관리자로부터 이용제재 받고 있는 유저입니다.", HttpStatus.BAD_REQUEST, "/", 103);
        ...
        if(joinPoint.getArgs()[1] != null){ // login my user id
            ...
            if (loginUser.getBlockUsers().contains(blockUser))
                throw new CustomControllerExecption("차단한 유저에 대한 정보를 조회할 수 없습니다.", HttpStatus.BAD_REQUEST, "/", 104);

            if (loginUser.getUserBlockedMe().contains(blockedUser))
                throw new CustomControllerExecption("나를 차단한 유저의 정보를 조회할 수 없습니다.", HttpStatus.BAD_REQUEST, "/", 105);
        }
    }
