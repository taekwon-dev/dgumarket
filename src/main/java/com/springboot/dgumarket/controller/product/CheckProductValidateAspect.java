...
public class CheckProductValidateAspect {
    ...
    public void checkProductValidateAOP(JoinPoint joinPoint) throws CustomControllerExecption {
        ...
        if (product.getMember().getIsWithdrawn() == 1) throw new CustomControllerExecption("물건의 판매자가 탈퇴하여 물건을 조회할 수 없습니다.", HttpStatus.BAD_REQUEST, "/", 102);
        if (product.getMember().getIsEnabled() == 1) throw new CustomControllerExecption("물건의 판매자가 관리자로 부터 이용제재조치를 받고 있어 물건을 조회할 수 없습니다.", HttpStatus.BAD_REQUEST, "/", 103);
        ...
        if(joinPoint.getArgs()[0] != null){
            ...
            if (loginUser.getBlockUsers().contains(blockUser))
                throw new CustomControllerExecption("차단한 유저의 물건을 조회할 수 없습니다.", HttpStatus.BAD_REQUEST, "/", 104);
            if (loginUser.getUserBlockedMe().contains(blockedUser))
                throw new CustomControllerExecption("나를 차단한 유저의 물건을 조회할 수 없습니다.", HttpStatus.BAD_REQUEST, "/", 105);
        }
