...
public class ProductValidationForChatRoomAspect {
    ...
    public Object checkDeleted(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        product.orElseThrow(()->new CustomControllerExecption("판매자에 의해 삭제처리된 중고물품 입니다.", HttpStatus.NOT_FOUND));
        ...
        if(product.get().getMember().getIsEnabled()==1){ // 이용제재
            throw new CustomControllerExecption("관리자에 의해 이용제재 받고 있는 유저의 물건은 조회할 수 없습니다.", HttpStatus.BAD_REQUEST);
        }

        if(product.get().getProductStatus() == 1){ // 유저 물건 자체 삭제
            throw new CustomControllerExecption("판매자에 의해 삭제처리된 중고물품 입니다.", HttpStatus.NOT_FOUND);
        }

        if(product.get().getProductStatus() == 2){ // 관리자에 의한 블라인드 처리
            throw new CustomControllerExecption("해당 중고물품은 관리자에 의해 비공개 처리되었습니다.", HttpStatus.NOT_FOUND);
        }
